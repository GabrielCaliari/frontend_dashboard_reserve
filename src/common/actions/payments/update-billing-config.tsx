...
import type { UpdateBillingConfigDto, TenantBillingConfig } from '@/src/common/@types/@payments';
...
export async function updateBillingConfigAction(
  ...
  try {
    ...
    throw new Error(error?.response?.data?.message || 'Failed to update billing config');
  }
}
