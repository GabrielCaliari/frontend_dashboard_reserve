...
export async function archivePlanAction(id: string): Promise<ArchivePlanResponse> {
  try {
    const tenantId = await getTenantIdFromCookie();

    if (!tenantId) {
      throw new Error('Nenhum tenant selecionado. Selecione um tenant antes de arquivar um plano.');
    }
    ...
    const message =
      error?.response?.data?.message || error?.message || 'Failed to archive plan';
