...
import type { CreateBillingConfigDto, TenantBillingConfig } from '@/src/common/@types/@payments';
...
export async function createBillingConfigAction(
  ...
  try {
    ...
    const message = error?.response?.data?.message || error?.message || 'Failed to create billing config';
