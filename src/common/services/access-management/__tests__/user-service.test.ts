...
describe('User Service', () => {
  ...
  describe('fetchUserById', () => {
    it('should fetch user details by ID', async () => {
      ...
      vi.mocked(accessManagementApiClient.get).mockResolvedValue({ data: mockUser });
  ...
  describe('updateUser', () => {
    it('should update user with partial data', async () => {
      ...
      vi.mocked(accessManagementApiClient.patch).mockResolvedValue({ data: mockUpdatedUser });

      const result = await updateUser(1, updateData);

      expect(accessManagementApiClient.patch).toHaveBeenCalledWith('/users/1', updateData);
    ...
    it('should handle 409 conflict errors for duplicate email', async () => {
      ...
      vi.mocked(accessManagementApiClient.patch).mockRejectedValue(conflictError);
  ...
  describe('deactivateUser', () => {
    it('should deactivate an active user', async () => {
      ...
      vi.mocked(accessManagementApiClient.patch).mockResolvedValue({ data: mockDeactivatedUser });

      const result = await deactivateUser(1);

      expect(accessManagementApiClient.patch).toHaveBeenCalledWith('/users/1/deactivate');
