...
describe('Admin Service', () => {
  ...
  describe('fetchAdminById', () => {
    it('should fetch admin details by ID', async () => {
      ...
      vi.mocked(accessManagementApiClient.get).mockResolvedValue({ data: mockAdmin });
  ...
  describe('createAdmin', () => {
    it('should create a new admin with valid data', async () => {
      ...
      vi.mocked(accessManagementApiClient.post).mockResolvedValue({ data: mockCreatedAdmin });

      const result = await createAdmin(createData);

      expect(accessManagementApiClient.post).toHaveBeenCalledWith('/admins', createData);
    ...
    it('should handle 400 validation errors', async () => {
      ...
      vi.mocked(accessManagementApiClient.post).mockRejectedValue(validationError);
  ...
  describe('updateAdmin', () => {
    it('should update admin with partial data', async () => {
      ...
      vi.mocked(accessManagementApiClient.patch).mockResolvedValue({ data: mockUpdatedAdmin });

      const result = await updateAdmin(1, updateData);

      expect(accessManagementApiClient.patch).toHaveBeenCalledWith('/admins/1', updateData);
  ...
  describe('activateAdmin', () => {
    it('should activate an inactive admin', async () => {
      ...
      vi.mocked(accessManagementApiClient.patch).mockResolvedValue({ data: mockActivatedAdmin });

      const result = await activateAdmin(1);

      expect(accessManagementApiClient.patch).toHaveBeenCalledWith('/admins/1/activate');
  ...
  describe('deactivateAdmin', () => {
    it('should deactivate an active admin', async () => {
      ...
      vi.mocked(accessManagementApiClient.patch).mockResolvedValue({ data: mockDeactivatedAdmin });

      const result = await deactivateAdmin(1);

      expect(accessManagementApiClient.patch).toHaveBeenCalledWith('/admins/1/deactivate');
  ...
  describe('deleteAdmin', () => {
    it('should delete an admin successfully', async () => {
      ...
      expect(accessManagementApiClient.delete).toHaveBeenCalledWith('/admins/1');
    });
