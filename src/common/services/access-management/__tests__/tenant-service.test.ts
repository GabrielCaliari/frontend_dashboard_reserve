...
import type { Tenant, PaginatedResponse, CreateTenantDto, UpdateTenantDto } from '@/src/common/@types/@access-management';
...
describe('Tenant Service', () => {
  ...
  describe('fetchTenants', () => {
    it('should fetch paginated list of tenants with default parameters', async () => {
      ...
      vi.mocked(accessManagementApiClient.get).mockResolvedValue({ data: mockResponse });
  ...
  describe('fetchTenantById', () => {
    it('should fetch tenant details by ID with assigned admins', async () => {
      ...
      vi.mocked(accessManagementApiClient.get).mockResolvedValue({ data: mockTenant });
  ...
  describe('createTenant', () => {
    it('should create a new tenant with valid data', async () => {
      ...
      vi.mocked(accessManagementApiClient.post).mockResolvedValue({ data: mockCreatedTenant });

      const result = await createTenant(createData);

      expect(accessManagementApiClient.post).toHaveBeenCalledWith('/tenants', createData);
    ...
    it('should handle 409 conflict errors for duplicate slug', async () => {
      ...
      vi.mocked(accessManagementApiClient.post).mockRejectedValue(conflictError);
  ...
  describe('updateTenant', () => {
    it('should update tenant with partial data', async () => {
      ...
      vi.mocked(accessManagementApiClient.patch).mockResolvedValue({ data: mockUpdatedTenant });

      const result = await updateTenant(1, updateData);

      expect(accessManagementApiClient.patch).toHaveBeenCalledWith('/tenants/1', updateData);
  ...
  describe('activateTenant', () => {
    it('should activate an inactive tenant', async () => {
      ...
      vi.mocked(accessManagementApiClient.patch).mockResolvedValue({ data: mockActivatedTenant });

      const result = await activateTenant(1);

      expect(accessManagementApiClient.patch).toHaveBeenCalledWith('/tenants/1/activate');
  ...
  describe('deactivateTenant', () => {
    it('should deactivate an active tenant', async () => {
      ...
      vi.mocked(accessManagementApiClient.patch).mockResolvedValue({ data: mockDeactivatedTenant });

      const result = await deactivateTenant(1);

      expect(accessManagementApiClient.patch).toHaveBeenCalledWith('/tenants/1/deactivate');
  ...
  describe('deleteTenant', () => {
    it('should delete a tenant successfully', async () => {
      vi.mocked(accessManagementApiClient.delete).mockResolvedValue({ data: undefined });

      await deleteTenant(1);

      expect(accessManagementApiClient.delete).toHaveBeenCalledWith('/tenants/1');
    });
    ...
    it('should handle 404 when deleting non-existent tenant', async () => {
      ...
      vi.mocked(accessManagementApiClient.delete).mockRejectedValue(notFoundError);
