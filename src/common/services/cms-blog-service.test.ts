...
describe("CMS Blog Service", () => {
  ...
  describe("updateBlog", () => {
    ...
    it("should handle error for invalid blog ID during update", async () => {
      ...
      await expect(updateBlog(999, updateData)).rejects.toThrow(
        "Blog not found",
      );
  ...
  describe("regenerateBlogSecretKey", () => {
    ...
    it("should handle error for invalid blog ID during key regeneration", async () => {
      ...
      await expect(regenerateBlogSecretKey(999)).rejects.toThrow(
        "Blog not found",
      );
      expect(cmsApiClient.post).toHaveBeenCalledWith(
        "/blogs/999/regenerate-key",
      );
    });
  });
});
