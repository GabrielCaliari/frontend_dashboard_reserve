...
export async function getBillingConfigService(): Promise<TenantBillingConfig> {
  const response = await apiClient.get<TenantBillingConfig>('/payments/billing-config');
...
export async function createBillingConfigService(
  ...
  const response = await apiClient.post<TenantBillingConfig>('/payments/billing-config', data);
  return response.data;
}
...
export async function updateBillingConfigService(
  ...
  const response = await apiClient.patch<TenantBillingConfig>('/payments/billing-config', data);
  return response.data;
}
