...
export function useInsertOtaData(clientId: string) {
  ...
  return useMutation({
    mutationFn: (data: InsertOtaDataDto) => hotelPortalService.insertOtaData(clientId, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['hotel-portal', 'ota-data', clientId] });
      qc.invalidateQueries({ queryKey: ['hotel-portal', 'dashboard', clientId] });
    },
  });
}
