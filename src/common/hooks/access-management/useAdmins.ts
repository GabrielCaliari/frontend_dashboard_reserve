...
export interface UseAdminsParams { page?: number; perPage?: number; search?: string; enabled?: boolean; }
...
export interface UseAdminByIdParams { id: string; enabled?: boolean; }
...
export function useCreateAdmin() {
  ...
  return useMutation<Admin, AxiosError<ApiErrorResponse>, CreateAdminDto>({
    ...
    onError: (error, _, context: any) => {
      if (context?.previousAdmins) { context.previousAdmins.forEach(([k, d]: any) => queryClient.setQueryData(k, d)); }
      toast.error(mapErrorMessage(error, 'Failed to create admin'));
    },
  });
}

export interface UpdateAdminParams { id: string; data: Omit<UpdateAdminDto, 'role'>; }
...
export function useUpdateAdmin() {
  ...
  return useMutation<Admin, AxiosError<ApiErrorResponse>, UpdateAdminParams>({
    ...
    onMutate: async ({ id, data }) => {
      ...
      queryClient.setQueriesData<PaginatedResponse<Admin>>(
        { queryKey: adminKeys.lists() },
        (old) => old ? { ...old, data: old.data.map((a) => a.id === id ? { ...a, ...data, updated_at: new Date().toISOString() } : a) } : old,
      );
      queryClient.setQueryData<Admin>(adminKeys.detail(id), (old) => old ? { ...old, ...data, updated_at: new Date().toISOString() } : old);
...
export interface UpdateAdminRoleParams { id: string; role: AdminRole; }

export function useUpdateAdminRole() {
  const queryClient = useQueryClient();
  return useMutation<Admin, AxiosError<ApiErrorResponse>, UpdateAdminRoleParams>({
    ...
    onMutate: async ({ id, role }) => {
      ...
      queryClient.setQueriesData<PaginatedResponse<Admin>>(
        { queryKey: adminKeys.lists() },
        (old) => old ? { ...old, data: old.data.map((a) => a.id === id ? { ...a, role, updated_at: new Date().toISOString() } : a) } : old,
      );
      queryClient.setQueryData<Admin>(adminKeys.detail(id), (old) => old ? { ...old, role, updated_at: new Date().toISOString() } : old);
      return { previousLists, previousDetail, id };
    },
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({ queryKey: adminKeys.lists() });
...
export interface ToggleAdminStatusParams { id: string; isActive: boolean; }

export function useToggleAdminStatus() {
  const queryClient = useQueryClient();
  return useMutation<Admin, AxiosError<ApiErrorResponse>, ToggleAdminStatusParams>({
    mutationFn: ({ id, isActive }) => isActive ? deactivateAdmin(id) : activateAdmin(id),
    onMutate: async ({ id, isActive }) => {
      ...
      queryClient.setQueriesData<PaginatedResponse<Admin>>(
        { queryKey: adminKeys.lists() },
        (old) => old ? { ...old, data: old.data.map((a) => a.id === id ? { ...a, is_active: newStatus, updated_at: new Date().toISOString() } : a) } : old,
      );
      queryClient.setQueryData<Admin>(adminKeys.detail(id), (old) => old ? { ...old, is_active: newStatus, updated_at: new Date().toISOString() } : old);
      return { previousLists, previousDetail, id };
    },
    onSuccess: (_, { isActive }) => {
      queryClient.invalidateQueries({ queryKey: adminKeys.lists() });
      toast.success(isActive ? 'Admin deactivated successfully' : 'Admin activated successfully');
    },
    onError: (error, _, context: any) => {
      if (context?.previousLists) { context.previousLists.forEach(([k, d]: any) => queryClient.setQueryData(k, d)); }
      if (context?.previousDetail && context?.id) { queryClient.setQueryData(adminKeys.detail(context.id), context.previousDetail); }
...
export function useDeleteAdmin() {
  ...
  return useMutation<void, AxiosError<ApiErrorResponse>, string>({
    ...
    onMutate: async (id) => {
      ...
      queryClient.setQueriesData<PaginatedResponse<Admin>>(
        { queryKey: adminKeys.lists() },
        (old) => old ? { ...old, data: old.data.filter((a) => a.id !== id), meta: { ...old.meta, total: old.meta.total - 1 } } : old,
      );
