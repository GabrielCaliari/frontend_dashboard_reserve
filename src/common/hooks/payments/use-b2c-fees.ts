...
export function useUpsertGlobalFees(messages?: ToastMessages) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['b2c-cost'], exact: false, refetchType: 'all' });
      toast.success(messages?.success ?? 'Global fees updated');
    },
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || messages?.error || 'Error updating global fees');
    },
  });
}
...
export function useUpsertB2BGlobalFees(messages?: ToastMessages) {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['b2b-cost'], exact: false, refetchType: 'all' });
      toast.success(messages?.success ?? 'Global fees updated');
    },
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || messages?.error || 'Error updating global fees');
...
export function useCreateCategory(messages?: ToastMessages) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: { name: string; slug: string; chipCostPercent?: number | null; shippingFee?: number | null }) =>
    ...
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || messages?.error || 'Error creating category');
    },
  });
}
...
export function useUpdateCategory(messages?: ToastMessages) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: { name?: string; chipCostPercent?: number | null; shippingFee?: number | null; active?: boolean } }) =>
    ...
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || messages?.error || 'Error updating category');
    },
  });
}
...
export function useDeleteCategory(messages?: ToastMessages) {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || messages?.error || 'Error removing category');
    },
  });
}
...
export function useUpdateProductFees(messages?: ToastMessages) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ productId, data }: { productId: string; data: { categoryId?: string | null; chipCostOverride?: number | null; shippingFeeOverride?: number | null; requiresShipping?: boolean } }) =>
      b2cProductsService.updateProductFees(productId, data),
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['b2c-products'] });
      queryClient.invalidateQueries({ queryKey: ['b2c-cost', variables.productId], exact: true, refetchType: 'all' });
      toast.success(messages?.success ?? 'Product fees updated');
    },
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || messages?.error || 'Error updating product fees');
    },
  });
}
...
export function useUpdateB2BProductFees(messages?: ToastMessages) {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['b2b-products'] });
      queryClient.invalidateQueries({ queryKey: ['b2b-cost', variables.productId], exact: true, refetchType: 'all' });
      toast.success(messages?.success ?? 'Product fees updated');
    },
    onError: (error: any) => {
      toast.error(error?.response?.data?.message || messages?.error || 'Error updating product fees');
