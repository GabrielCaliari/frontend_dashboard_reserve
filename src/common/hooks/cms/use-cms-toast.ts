...
import { CMSError, getErrorMessage } from '@/src/common/utils/cms-error-handler';
...
export function useCMSToast() {
  ...
  const blogCreated = () => showSuccess('Blog created successfully', 'You can now start adding articles.');
  const blogUpdated = () => showSuccess('Blog updated successfully');
  const blogDeleted = () => showSuccess('Blog deleted successfully', 'All associated articles have been removed.');
  const blogKeyRegenerated = () => showSuccess('Secret key regenerated', 'The old key is no longer valid.');

  // Article operation toasts
  const articleCreated = () => showSuccess('Article created successfully', 'Your article has been saved as a draft.');
