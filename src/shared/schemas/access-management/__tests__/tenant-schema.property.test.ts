...
describe("Tenant Schema Property Tests", () => {
  describe("Property 9: Slug format validation", () => {
    ...
    it("should reject slugs shorter than 2 characters", () => {
      fc.assert(
        fc.property(fc.string({ maxLength: 1 }), (shortSlug) => {
          const result = createTenantSchema.safeParse({
            name: "Test Tenant",
            slug: shortSlug,
            domain: "example.com",
  ...
  describe("Domain validation", () => {
    ...
    it("should accept valid domain formats", () => {
      fc.assert(
        fc.property(fc.domain(), (validDomain) => {
          const result = createTenantSchema.safeParse({
            name: "Test Tenant",
            slug: "test-tenant",
            domain: validDomain,
        ...
        fc.property(
          ...
          (longDomain) => {
            ...
            if (!result.success) {
              ...
              expect(
                domainErrors.some((err) =>
                  err.message.includes("255 characters"),
                ),
              ).toBe(true);
            }
          },
        ...
        fc.property(
          ...
          (longName) => {
            ...
            if (!result.success) {
              ...
              expect(
                nameErrors.some((err) =>
                  err.message.includes("100 characters"),
                ),
              ).toBe(true);
            }
          },
