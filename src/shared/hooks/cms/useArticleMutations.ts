...
import {
  ...
  archiveArticle,
  unarchiveArticle,
} from '@/src/modules/cms/infrastructure/article-adapters';
...
export function useUpdateArticle() {
  ...
  return useMutation({
    ...
    }) => updateArticle(String(articleId), data),
    onMutate: async ({ blogId, articleId, data }) => {
      ...
      queryClient.setQueryData<Article>(
        ARTICLE_QUERY_KEYS.detail(tenantId, blogId, articleId),
        (old) => (old ? { ...old, ...data, updated_at: new Date().toISOString() } as Article : old),
      );
