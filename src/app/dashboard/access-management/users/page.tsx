...
export default function UserListPage() {
  ...
  const { data, isLoading, error } = useUsers({ page: currentPage, perPage, search: searchTerm || undefined });
  ...
  const handleFormSubmit = async (formData: UpdateUserFormData) => {
    ...
    try {
      await updateUserMutation.mutateAsync({ id: selectedUser.id, data: formData as UpdateUserDto });
      ...
      <LayoutScopeRoot routeActive="users">
        <div className="p-6">
          <div className="bg-danger/10 border border-danger/20 rounded-lg p-6 text-center">
            <h2 className="text-xl font-semibold text-danger mb-2">{t("users.management.errorTitle")}</h2>
            <p className="text-danger-500 mb-4">{error.message || t("users.management.errorMessage")}</p>
            <Button color="danger" variant="flat" onPress={() => window.location.reload()}>{t("users.management.retry")}</Button>
          </div>
        </div>
      </LayoutScopeRoot>
    );
  ...
  return (
    <>
      <LayoutScopeRoot routeActive="users">
        <div className="p-6">
          <Breadcrumbs items={[{ label: t("breadcrumbs.dashboard"), href: "/dashboard" }, { label: t("breadcrumbs.accessManagement") }, { label: t("breadcrumbs.users") }]} />

          <div className="mb-6">
            <h1 className="text-3xl font-bold text-foreground mb-1">{t("users.management.title")}</h1>
            <p className="text-muted-foreground">{t("users.management.description")}</p>
          </div>

          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="flex-1">
              <SearchInput value={searchTerm} onChange={handleSearchChange} placeholder={t("users.management.searching")} className="max-w-md" />
            </div>
          ...
          <UserTable
            ...
            onRowClick={(id) => router.push(`/dashboard/access-management/users/${id}`)}
          ...
          {!isLoading && filteredUsers.length > 0 && (
            <div className="mt-6 flex justify-between items-center">
              <p className="text-sm text-muted-foreground">{t("users.management.showing", { count: filteredUsers.length, total: data?.meta.total || 0 })}</p>
              <PaginationControls
      ...
      {selectedUser && (
        <UserFormModal
          ...
          onClose={() => { setIsFormModalOpen(false); setSelectedUser(undefined); }}
          user={selectedUser}
      ...
      <ConfirmationDialog
        ...
        confirmText={confirmDialog.variant === "danger" ? t("users.management.confirmDelete") : t("users.management.confirmAction")}
