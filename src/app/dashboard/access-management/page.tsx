...
export default function AccessManagementOverviewPage() {
  ...
  return (
    <LayoutScopeRoot routeActive="access-management">
      <div>
        <Breadcrumbs items={[{ label: t("breadcrumbs.dashboard"), href: "/dashboard" }, { label: t("breadcrumbs.accessManagement") }]} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {sections.map((section) => (
            <Card
              ...
              className="border border-divider hover:border-primary/30 transition-all hover:shadow-lg hover:shadow-primary/5 group"
            >
              <CardBody className="p-6">
                <div className={`inline-flex p-3 rounded-xl border ${section.color} mb-4`}>
                  {section.icon}
                </div>
                ...
                {section.stat !== undefined && section.stat !== null && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="font-semibold text-foreground">{section.stat}</span>
