...
export default function LeadsPage() {
  ...
  const hasAnyFilter =
    hasCollectionFilter || statusFilter !== "" || originFilter !== "";
  ...
  const {
    data: collectionLeadsData,
    isLoading: isLoadingCollection,
    refetch: refetchCollection,
  } = useGetCollectionLeads({
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      if (hasCollectionFilter) {
        const res = await getCollectionLeadsService(collectionFilter, {
          page: 1,
          limit: totalCount,
        });
