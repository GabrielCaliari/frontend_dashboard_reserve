...
import { DialogHeader, DialogTitle, DialogFooter } from"@/src/presentation/components/atoms/shadcn-ui/dialog"
import { Input } from"@/src/presentation/components/atoms/shadcn-ui/input"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from"@/src/presentation/components/atoms/shadcn-ui/select"
...
export function FunnelBuilder({ onComplete, onCancel, isSubmitting }: FunnelBuilderProps) {
 ...
 const removeStepAndChildren = (id: string): string[] => {
 const step = steps.find((s) => s.id === id)
 if (!step) return []

 const childrenIds = step.children
 const childrenToRemove: string[] = childrenIds.flatMap((childId: string) => removeStepAndChildren(childId))
 ...
 onValueChange={(value) => updateStep(step.id, { condition: { type: step.condition?.type ?? "opened", ...step.condition, target: value } })}
 >
 ...
 onChange={(e) =>
 updateStep(step.id, { delay: { ...(step.delay ?? { days: 0, hours: 0 }), days: Number.parseInt(e.target.value) || 0 } })
 }
 className="mt-1"
 />
 </div>
 ...
 onChange={(e) =>
 updateStep(step.id, { delay: { ...(step.delay ?? { days: 0, hours: 0 }), hours: Number.parseInt(e.target.value) || 0 } })
 }
 className="mt-1"
 />
 </div>
