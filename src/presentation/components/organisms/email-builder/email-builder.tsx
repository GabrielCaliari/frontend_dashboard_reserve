...
import EmailSettingsModal, { EmailMetadata, type EmailSettings } from"./email-settings-modal"
import { convertHtmlToMarkdown, convertHeaderToMarkdown, convertFooterToMarkdown } from"@/src/shared/lib/html-to-markdown"
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from"@/src/presentation/components/atoms/shadcn-ui/dropdown-menu"
import { IEmailComponent, IEmailTemplate } from"@/src/shared/domain/types/@email-builder"
...
import { updateCopyEmailByCampaignBatchIdService } from"@/src/modules/mailer/infrastructure/adapters"
...
export default function EmailBuilder({ email, campaignBatchId, campaign, primaryCopy, isPrimaryCopy = true }: { email: string, campaignBatchId?: string, campaign?: IEmailCampaign, primaryCopy: IEmail, isPrimaryCopy?: boolean }) {
 ...
 const [emailMetadata, setEmailMetadata] = useState<EmailMetadata>({ subject:"", preheader:"", fromName:"" })
 ...
 const response = await updateCopyEmailByCampaignBatchIdService(String(campaignBatchId), data)
 ...
 const { isNew, ...cleanComponent } = component as IEmailComponent & { isNew?: boolean }
 ...
 setEmailComponents(emailComponents.map((comp) => (comp.id === updatedComponent.id ? updatedComponent : comp)))
 ...
 const moveMultipleComponents = (dragIndices: number[], targetIndex: number) => {
 ...
 setSelectedBlocks((prev) => (prev.includes(id) ? prev.filter((blockId) => blockId !== id) : [...prev, id]))
 }
 ...
 const generateEmailHtml = (template: IEmailTemplate, str: boolean = false): string => {
 ...
 const componentPadding = component.padding || emailSettings.defaultPadding
 switch (component.type) {
 case"text":
 content +=`
 <tr>
 ...
 ${content.replace('<!-- HEADER AQUI -->','')
 .replace('<!-- FOOTER AQUI -->', emailSettings.useFooter ?`<tr><td>${emailSettings.footerHtml}</td></tr>` :'')}
 </body>
 </html>`
 ...
 if (!td) return;
 // TEXT
 const div = td.querySelector('div[data-text]') as HTMLElement | null;
 ...
 // IMAGE
 const img = td.querySelector('img[data-image]') as HTMLImageElement | null;
 if (img) {
 blocks.push({
 ...
 // BUTTON
 const aButton = td.querySelector('a[data-button]') as HTMLAnchorElement | null;
 if (aButton && aButton.textContent && aButton.style.display?.includes('inline-block')) {
 blocks.push({
 ...
 // LINK
 const aLink = td.querySelector('a[data-link]') as HTMLAnchorElement | null;
 if (aLink && (!aLink.style.display || aLink.style.display ==='inline-block' || aLink.style.display ==='')) {
 blocks.push({
 ...
 // DIVIDER
 const hr = td.querySelector('hr[data-divider]') as HTMLHRElement | null;
 if (hr) {
 blocks.push({
 ...
 useEffect(() => {
 if (primaryCopy && primaryCopy.html_content && emailComponents.length === 0) {
 ...
 <EmailSaveModal primaryCopy={primaryCopy} email={email} isOpen={saveModalOpen} onClose={() => setSaveModalOpen(false)} onSave={handleSaveMetadata} />
