...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from"@/src/presentation/components/atoms/shadcn-ui/dialog"
...
export default function EmailSettingsModal({ isOpen, onClose, settings, onSaveSettings }: EmailSettingsModalProps) {
 ...
 links.items.forEach((link, index) => {
 html +=`<a href="${link.url}" style="color: ${links.color}; text-decoration: none; margin: 0 5px;">${link.text}</a>`
 if (index < links.items.length - 1) {
 html +=` |`
 }
 })
 html +=`</div>`
 ...
 html +=`<div style="margin-top: 10px;">`
 socialLinks.items.forEach((socialLink) => {
 html +=`<a href="${socialLink.url}" title="${socialLink.name}" style="display: inline-block; width: ${socialLinks.size}; height: ${socialLinks.size}; line-height: ${socialLinks.size}; text-align: center; border-radius: 50%; background-color: #007bff; color: #ffffff; text-decoration: none; margin: 0 5px;">${socialLink.name.charAt(0)}</a>`
 })
 html +=`</div>`
 }
