    ...
    <Card className="bg-card border border-border">
      <CardBody className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          {coupon.minOrderAmount !== null && (
            <div>
              ...
              <p className="text-foreground">
                {(coupon.minOrderAmount / 100).toLocaleString(locale, {
                  style: "currency",
                  currency: "BRL",
                })}
              </p>
          ...
          <div>
            ...
            <p className="text-foreground">
              {coupon.cumulative
                ? t("summaryCumulativeYes")
                : t("summaryCumulativeNo")}
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-4 text-xs text-muted-foreground pt-1 border-t border-border">
          <span>
            {t("summaryCreatedAt", {
              date: new Date(coupon.createdAt).toLocaleDateString(locale),
            })}
          </span>
          <span>
            {t("summaryUpdatedAt", {
              date: new Date(coupon.updatedAt).toLocaleDateString(locale),
            })}
          </span>
        </div>
      </CardBody>
