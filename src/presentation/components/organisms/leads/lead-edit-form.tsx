...
import { useUpdateLead } from "@/src/shared/hooks/leads/use-update-lead";
import type {
  Lead,
  UpdateLeadDto,
  EOriginLead,
...
export function LeadEditForm({ lead, onSuccess, onCancel }: LeadEditFormProps) {
  ...
  const onSubmit = (values: UpdateLeadDto) => {
    ...
    if (values.origin_font !== undefined && values.origin_font !== "")
      payload.origin_font = values.origin_font;
    if (values.description !== undefined && values.description !== "")
      payload.description = values.description;

    updateLead.mutate({ id: lead.id, data: payload }, { onSuccess });
  };
