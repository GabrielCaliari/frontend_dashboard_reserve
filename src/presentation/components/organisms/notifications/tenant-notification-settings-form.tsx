...
const EVENT_KEYS: Array<{ key: string; label: string; description: string }> = [
  {
    key: "lead.created",
    label: "Novo lead",
    description: "Notifica quando um novo lead é registrado",
  },
  {
    key: "subscription.expiring",
    label: "Assinatura vencendo",
    description: "Notifica quando a assinatura está próxima do vencimento",
  },
  {
    key: "subscription.expired",
    label: "Assinatura expirada",
    description: "Notifica quando a assinatura expira",
  },
];

const CRON_KEYS: Array<{ key: string; label: string; description: string }> = [
  {
    key: "subscription.expiry_check",
    label: "Verificação diária de vencimento",
    description:
      "Job diário que alerta sobre assinaturas próximas do vencimento",
  },
  {
    key: "subscription.expired_check",
    label: "Verificação diária de expiradas",
    description: "Job diário que alerta sobre assinaturas já expiradas",
  },
];
...
export function TenantNotificationSettingsForm({
  tenantId,
  isSuperAdmin,
}: Props) {
  ...
  const toggle = async (
    field: "email_enabled" | "inapp_enabled",
    value: boolean,
  ) => {
    try {
  ...
  const toggleEvent = async (key: string, value: boolean) => {
    try {
      await updateSettings({
        event_settings: { ...(settings?.event_settings ?? {}), [key]: value },
      });
    } catch {
      toast.error("Erro ao salvar configuração.");
    }
  ...
  const toggleCron = async (key: string, value: boolean) => {
    try {
      await updateSettings({
        cron_settings: { ...(settings?.cron_settings ?? {}), [key]: value },
      });
    } catch {
      toast.error("Erro ao salvar configuração.");
    }
              ...
              <div className="flex flex-col gap-0.5">
                <div className="flex items-center gap-2">
                  ...
                  <Chip
                    ...
                    variant="flat"
                    color="default"
                    className="h-4 text-[10px] px-1 font-mono"
                  >
                    {key}
                  </Chip>
                </div>
