...
export function SubscriptionsSection() {
  ...
  const { data: billingConfig, isLoading: isLoadingConfig } =
    useBillingConfig();
  ...
  useEffect(() => {
    ...
    if (success === "1") {
      toast.success(
        "Pagamento processado! Aguardando confirmação do Stripe...",
      );
  ...
  const handleSubscribe = async (planId?: string) => {
    ...
    if (activePlans.length === 0) {
      toast.error(
        "Nenhum plano ativo encontrado. Crie um plano primeiro em Products.",
      );
      return;
    }
    ...
    let selectedPlan =
      activePlans.find((p) => p.id === planId) || activePlans[0];
    ...
    if (!planId && billingConfig && activePlans.length > 1) {
      ...
      const matchingPlan = activePlans.find(
        (p) => p.billing_interval === preferredInterval,
      );
      if (matchingPlan) {
    ...
    try {
      ...
      const { checkoutUrl } = await createCheckout.mutateAsync({
        ...
        trialPeriodDays:
          selectedPlan.trial_days > 0 ? selectedPlan.trial_days : undefined,
      });
      window.location.href = checkoutUrl;
    } catch (error: any) {
      console.error("Checkout error:", error);
      const errorMessage =
        error?.response?.data?.message || error?.message || t("checkoutError");
      ...
      {plans.length === 0 && (
        <div className="flex items-start gap-3 p-4 rounded-xl bg-yellow-950/30 border border-yellow-800/50">
          ...
          <div>
            ...
            <p className="text-xs text-yellow-400 mt-0.5">
              Crie planos em Products para começar a aceitar assinaturas.
            </p>
          </div>
        </div>
      )}
      ...
      {!subscription && plans.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 gap-4 rounded-xl bg-default-100 border border-[#2a2a3e]">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Crie planos em Products para começar
            </p>
          </div>
        </div>
      )}
