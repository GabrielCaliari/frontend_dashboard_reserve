...
function CategoryRow({
  ...
  const [chipCost, setChipCost] = useState(
    category.chipCostPercent != null ? String(category.chipCostPercent) : "",
  );
  const [shipping, setShipping] = useState(
    category.shippingFee != null
      ? String((category.shippingFee / 100).toFixed(2))
      : "",
  );
  ...
  const handleSave = () => {
    onUpdate(category.id, {
      ...
      shippingFee:
        shipping !== "" ? Math.round(parseFloat(shipping) * 100) : null,
    });
    setEditing(false);
  };
      ...
      <div className="rounded-xl bg-white/[0.04] border border-white/10 p-3 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Input
            ...
            placeholder={t("feesCategoriesInheritGlobal")}
            value={chipCost}
            onValueChange={setChipCost}
            type="number"
            min="0"
            step="0.1"
            startContent={<Percent className="w-3 h-3 text-muted-foreground" />}
            classNames={{ inputWrapper: "bg-white/[0.03] border-white/[0.07]" }}
          />
          <Input
            size="sm"
            ...
            min="0"
            step="0.01"
            startContent={<Truck className="w-3 h-3 text-muted-foreground" />}
            classNames={{ inputWrapper: "bg-white/[0.03] border-white/[0.07]" }}
          />
        </div>
        <div className="flex gap-2 justify-end">
          <Button size="sm" variant="flat" isIconOnly onPress={handleCancel}>
            <X className="w-3.5 h-3.5" />
          </Button>
          <Button size="sm" color="primary" isIconOnly onPress={handleSave}>
        ...
        <div className="flex items-center gap-2 mt-0.5">
          ...
          {category.shippingFee != null ? (
            <span className="text-[11px] text-blue-400/70">
              {t("feesShipping")}: R${(category.shippingFee / 100).toFixed(2)}
            </span>
          ) : (
            <span className="text-[11px] text-muted-foreground italic">
...
export function CategoriesModal({ isOpen, onClose }: CategoriesModalProps) {
  ...
  const handleCreate = async () => {
    ...
    await createMutation.mutateAsync({
      ...
      shippingFee:
        newShipping !== "" ? Math.round(parseFloat(newShipping) * 100) : null,
    });
    setNewName("");
    setNewChipCost("");
    setNewShipping("");
        ...
        <ModalBody className="py-4 space-y-3">
          ...
          <div className="rounded-xl bg-primary/5 border border-primary/10 p-3 space-y-3">
            <p className="text-xs font-semibold text-primary/80 uppercase tracking-wider">
              {t("feesCategoriesNewTitle")}
            </p>
            <Input
              size="sm"
              placeholder={t("feesCategoriesNamePlaceholder")}
              value={newName}
