...
export function CreateProductModal({
  ...
  const handleNameChange = (value: string) => {
    ...
    setForm((prev) => ({
      ...
      name: value,
      slug:
        !currentSlug || currentSlug === expectedSlug
          ? autoSlug(value)
          : currentSlug,
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (billingType === "b2b") {
        ...
        if (
          form.billingConfig.mode === "one_time_expiring" &&
          form.billingConfig.accessDuration
        ) {
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm text-foreground">
                    {t("physicalProductLabel")}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {t("physicalProductDesc")}
                  </p>
                </div>
              </div>
              <Switch
