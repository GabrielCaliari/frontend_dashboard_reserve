...
interface B2BGlobalFeesModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function B2BGlobalFeesModal({
  isOpen,
  onClose,
}: B2BGlobalFeesModalProps) {
  const t = useTranslations("payments.productsPage");
  const tCommon = useTranslations("common");
  const { data: config, isLoading } = useB2BGlobalFees();
  const upsert = useUpsertB2BGlobalFees({
  ...
  useEffect(() => {
    if (isOpen && config) {
      setChipCost(
        config.chipCostPercent > 0 ? String(config.chipCostPercent) : "",
      );
      ...
      <ModalContent>
        <ModalHeader className="flex items-center gap-2.5">
          ...
          <span className="text-sm font-semibold text-foreground">
            {t("feesGlobalModalTitle")} — One-time
          </span>
        </ModalHeader>
            ...
            <>
              ...
              <div className="flex gap-2 items-end">
                ...
                <Select
                  ...
                  aria-label="Currency"
                  classNames={{
                    trigger: "bg-white/[0.03] border-white/[0.07] h-[56px]",
                  }}
                >
