...
const UserTable: React.FC<UserTableProps> = ({
  ...
  const renderCell = (user: User, columnKey: React.Key) => {
    switch (columnKey) {
      ...
      case "email":
        return (
          <span className="text-sm text-foreground-400">{user.email}</span>
        );
      case "phone":
        return (
          ...
          >
            <Tooltip content={t("table.tooltips.viewDetails")}>
              <Button
                ...
                size="sm"
                variant="light"
                onPress={() => onRowClick(user.id)}
                aria-label={t("table.ariaLabels.viewUser")}
              >
            ...
            <Tooltip content={t("table.tooltips.editUser")}>
              <Button
                ...
                variant="light"
                onPress={() => onEdit(user)}
                aria-label={t("table.ariaLabels.editUser")}
              >
            ...
            {user.is_active && (
              <Tooltip content={t("table.tooltips.deactivateUser")}>
                <Button
                  ...
                  aria-label={t("table.ariaLabels.deactivateUser")}
                >
            ...
            <Tooltip content={t("table.tooltips.deleteUser")} color="danger">
              <Button
                ...
                aria-label={t("table.ariaLabels.deleteUser")}
              >
    ...
    return (
      <div className="text-center py-12">
        <p className="text-foreground-500 text-lg">
          {t("table.empty.noUsersFound")}
        </p>
        <p className="text-foreground-400 text-sm mt-2">
          {t("table.empty.usersWillAppear")}
        </p>
      </div>
    );
  }
