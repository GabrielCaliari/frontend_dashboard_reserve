...
const TenantTable: React.FC<TenantTableProps> = ({
  ...
  const renderCell = (tenant: Tenant, columnKey: React.Key) => {
    switch (columnKey) {
      case "id":
        return (
          <div className="flex items-center gap-1">
            ...
            <Tooltip
              content={copiedId === tenant.id ? "Copiado!" : "Copiar ID"}
            >
              <Button
                isIconOnly
                size="sm"
      ...
      case "slug":
        return (
          <span className="text-sm text-foreground-400 font-mono">
            {tenant.slug}
          </span>
        );
      case "domain":
        return (
          <span className="text-sm text-foreground-400">{tenant.domain}</span>
        );
      case "status":
          ...
          >
            <Tooltip content={t("table.tooltips.editTenant")}>
              <Button
                ...
                aria-label={t("table.ariaLabels.editTenant")}
            ...
            >
              <Button
                ...
                aria-label={
                  tenant.is_active
                    ? t("table.ariaLabels.deactivateTenant")
                    : t("table.ariaLabels.activateTenant")
                }
                isDisabled={tenant.scheduled_for_deletion}
              >
            ...
            {tenant.scheduled_for_deletion ? (
              <Tooltip
                content={t("table.tooltips.restoreTenant")}
                color="primary"
              >
                <Button
                  isIconOnly
                  ...
                  color="primary"
                  onPress={() =>
                    onRestore ? onRestore(tenant.id) : onDelete(tenant.id)
                  }
              ...
              <Tooltip
                content={t("table.tooltips.deleteTenant")}
                color="danger"
              >
                <Button
                  isIconOnly
    ...
    return (
      <div className="text-center py-12">
        <p className="text-foreground-500 text-lg">
          {t("table.empty.noTenantsFound")}
        </p>
        <p className="text-foreground-400 text-sm mt-2">
          {t("table.empty.createNewTenant")}
        </p>
      </div>
    );
  }
