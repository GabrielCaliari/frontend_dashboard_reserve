...
export function AdminFormModal({
  ...
  const [selectedNewRole, setSelectedNewRole] =
    useState<TenantScopedRoleType>(AdminRole.viewer);
        ...
        setTenantEntries(
          (resolvedAdmin.tenants ?? [])
            ...
            .map((rel) => ({
              ...
              originalRole: (rel.role as TenantScopedRoleType) ?? AdminRole.viewer,
            })),
        );
      } else if (!isEditMode) {
        reset({
  ...
  const assignedTenantIds = useMemo(
    () =>
      new Set(
        tenantEntries.filter((e) => !e.removed).map((e) => e.tenant_id),
      ),
    [tenantEntries],
  );
  ...
  const computeTenantChanges = useCallback((): TenantAssignmentChange => {
    ...
    for (const entry of tenantEntries) {
      ...
      } else if (!entry.isNew && !entry.removed && entry.originalRole !== undefined && entry.originalRole !== entry.role) {
  ...
  const hasTenantChanges = useMemo(() => {
    ...
    return changes.add.length > 0 || changes.remove.length > 0 || changes.updateRole.length > 0;
  ...
  const handleFormSubmit = async (
    ...
    try {
      const tenantChanges = isSuperAdmin && hasTenantChanges
        ? computeTenantChanges()
        : undefined;
  ...
  const handleTenantRoleChange = (
    ...
    setTenantEntries((prev) =>
      prev.map((e) =>
        e.tenant_id === tenantId ? { ...e, role: newRole } : e,
      ),
    );
  };
            ...
            <div className="space-y-5">
              ...
              {isSuperAdmin && (
                <div className="space-y-3 pt-3 border-t border-divider">
                  ...
                  {addingTenant && (
                    <div className="p-4 bg-content2 rounded-lg space-y-3 border border-default-200">
                      <Select
                      ...
                      <div>
                        <Select
                          ...
                          onSelectionChange={(keys) => {
                            const val = Array.from(keys)[0] as TenantScopedRoleType;
                  ...
                  {visibleEntries.length > 0 ? (
                    <div className="space-y-2">
                      {visibleEntries.map((entry) => (
                        <div
                          key={entry.tenant_id}
                          className="flex items-center justify-between p-3 bg-content2 rounded-lg border border-default-100"
                        >
                          ...
                          <div className="flex items-center gap-2 ml-3">
                            ...
                            <Button
                              ...
                              variant="light"
                              color="danger"
                              onPress={() => handleRemoveTenant(entry.tenant_id)}
