...
const AdminTable: React.FC<AdminTableProps> = ({
  ...
  const canActOn = (admin: Admin) =>
    isSuperAdmin ||
    (admin.role !== AdminRole.super_admin && admin.role !== AdminRole.owner);
  ...
  const renderCell = (admin: Admin, columnKey: React.Key) => {
    switch (columnKey) {
      ...
      case "email":
        return (
          <span className="text-sm text-foreground-400">{admin.email}</span>
        );
      case "role":
          ...
          >
            <Tooltip content={t("table.tooltips.viewDetails")}>
              <Button
                ...
                size="sm"
                variant="light"
                onPress={() => onRowClick(admin.id)}
                aria-label={t("table.ariaLabels.viewAdmin")}
              >
            ...
            <Tooltip
              content={
                !canAct
                  ? t("table.tooltips.insufficientPermissions") ||
                    "Insufficient permissions"
                  : t("table.tooltips.editAdmin")
              }
            >
              <Button
                isIconOnly
                ...
                isDisabled={!canAct || admin.scheduled_for_deletion}
                aria-label={t("table.ariaLabels.editAdmin")}
              >
            ...
            <Tooltip
              content={
                isSelf
                  ? t("table.tooltips.cannotDeactivateSelf")
                  : !canAct
                    ? t("table.tooltips.insufficientPermissions") ||
                      "Insufficient permissions"
                    : admin.is_active
                      ? t("table.tooltips.deactivate")
                      : t("table.tooltips.activate")
              }
            >
              <Button
                isIconOnly
                ...
                aria-label={
                  admin.is_active
                    ? t("table.ariaLabels.deactivateAdmin")
                    : t("table.ariaLabels.activateAdmin")
                }
              >
            ...
            {admin.scheduled_for_deletion ? (
              <Tooltip
                content={t("table.tooltips.restoreAdmin")}
                color="primary"
              >
                <Button
                  isIconOnly
                  ...
                  color="primary"
                  onPress={() =>
                    onRestore ? onRestore(admin.id) : onDelete(admin.id)
                  }
              ...
              <Tooltip
                content={
                  isSelf
                    ? t("table.tooltips.cannotDeleteSelf")
                    : !canAct
                      ? t("table.tooltips.insufficientPermissions") ||
                        "Insufficient permissions"
                      : t("table.tooltips.deleteAdmin")
                }
                color="danger"
              >
                <Button
    ...
    return (
      <div className="text-center py-12">
        <p className="text-foreground-500 text-lg">
          {t("table.empty.noAdminsFound")}
        </p>
        <p className="text-foreground-400 text-sm mt-2">
          {t("table.empty.createNewAdmin")}
        </p>
      </div>
    );
  }
  ...
  return (
    <Table
      ...
      classNames={{
        wrapper:
          " border border-divider bg-content1 rounded-lg overflow-x-auto",
    ...
    >
      <TableHeader columns={columns}>
        {(col) => (
          <TableColumn
            key={col.key}
            align={col.key === "actions" ? "center" : "start"}
          >
            {col.label}
          </TableColumn>
        )}
