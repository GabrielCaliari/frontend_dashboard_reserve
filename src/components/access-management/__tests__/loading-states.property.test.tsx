...
const loadingStateArbitrary = fc.constantFrom('idle','loading','success','error'
);

// Generator for mutation types
const mutationTypeArbitrary = fc.constantFrom('create','update','delete','activate','deactivate'
);
...
describe('Loading States and User Feedback Property Tests', () => {
 ...
 const loadingIndicator = table.querySelector('[data-testid="loading-spinner"]');
 
 if (isLoading) {
 expect(loadingIndicator).not.toBeNull();
 expect(loadingIndicator?.getAttribute('role')).toBe('status');
 expect(loadingIndicator?.getAttribute('aria-label')).toBe('Loading');
 } else {
 ...
 const skeletonRows = tbody.querySelectorAll('[data-testid="skeleton-row"]');
 ...
 label.textContent = validationError.field.charAt(0).toUpperCase() + validationError.field.slice(1);
 ...
 input.setAttribute('aria-describedby',`${validationError.field}-error`);
 ...
 expect(input.getAttribute('aria-describedby')).toBe(`${validationError.field}-error`);
 ...
 validationErrors.forEach(error => {
 const formGroup = form.querySelector(`[data-field="${error.field}"]`);
