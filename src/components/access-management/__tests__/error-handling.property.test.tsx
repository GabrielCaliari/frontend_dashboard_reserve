...
const apiErrorArbitrary = fc.record({
 code: fc.constantFrom('VALIDATION_ERROR','DUPLICATE_EMAIL','DUPLICATE_SLUG','NOT_FOUND','UNAUTHORIZED','FORBIDDEN','INTERNAL_ERROR'
 ),
 message: fc.string({ minLength: 10, maxLength: 100 }).filter(s => s.trim().length >= 10),
...
const networkErrorArbitrary = fc.constantFrom('NETWORK_ERROR','TIMEOUT','CONNECTION_REFUSED','DNS_LOOKUP_FAILED','ECONNABORTED'
);

// Generator for mutation types
const mutationTypeArbitrary = fc.constantFrom('create','update','delete','activate','deactivate','assign','remove'
);
...
describe('Error Handling and Recovery Property Tests', () => {
 ...
 expect(messageEl.getAttribute('data-error-code')).toBe(apiError.code);
 
 // Verify status code for server errors
 if (apiError.statusCode >= 500) {
 const statusEl = toast.querySelector('.status-code');
 expect(statusEl).not.toBeNull();
 expect(statusEl?.textContent).toContain(apiError.statusCode.toString());
 }
 ...
 Object.entries(additionalData.details).forEach(([field, messages]) => {
 ...
 const totalMessages = Object.values(additionalData.details).reduce(
 ...
 errorHandler.setAttribute('data-status-code', statusCode.toString());
 ...
 expect(errorHandler.getAttribute('data-status-code')).toBe(statusCode.toString());
 expect(errorHandler.getAttribute('data-error-type')).toBe(errorType);
 ...
 message.textContent ='Network connection failed. Please check your internet connection.';
 ...
 const networkMessages = {'NETWORK_ERROR':'Unable to connect to the server','TIMEOUT':'Request timed out - connection issue','CONNECTION_REFUSED':'Connection refused by server','DNS_LOOKUP_FAILED':'Could not resolve server address','ECONNABORTED':'Connection was aborted'
 };
 
 message.textContent = networkMessages[networkError as keyof typeof networkMessages] ||'Network error occurred';
 errorDisplay.appendChild(message);
 ...
 const retryButton = document.createElement('button');
 retryButton.textContent ='Retry';
 retryButton.setAttribute('data-retry-count', currentRetryCount.toString());
 retryButton.onclick = () => {
 currentRetryCount++;
 retryButton.setAttribute('data-retry-count', currentRetryCount.toString());
 };
 ...
 expect(retryButton.getAttribute('data-retry-count')).toBe(retryCount.toString());
 ...
 expect(input?.getAttribute('aria-describedby')).toBe(`${error.field}-error`);
 });
 ...
 Object.entries(apiResponse.details).forEach(([fieldName, messages]) => {
 const formGroup = form.querySelector(`[data-field="${fieldName}"]`);
 ...
 fc.property(
 fc.constantFrom('name','email','password','slug','domain'),
 fc.array(fc.string({ minLength: 5, maxLength: 50 }), { minLength: 2, maxLength: 4 }),
 ...
 expect(input.getAttribute('aria-invalid')).toBe('true');
 expect(errorEl.textContent).toBe(validationError.message);
 expect(errorEl.getAttribute('data-field-path')).toBe(validationError.field);
 ...
 expect(currentError.getAttribute('data-error-type')).toBe(networkError);
 ...
 expect(currentError.getAttribute('data-field')).toBe(validationError.field);
