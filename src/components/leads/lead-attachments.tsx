...
export function LeadAttachments({ leadId }: LeadAttachmentsProps) {
  ...
  return (
    <div className="space-y-4">
      {attachments.length === 0 ? (
        ...
        <ul className="space-y-2">
          {attachments.map((att) => (
            <li
              key={att.id}
              className="flex items-center justify-between rounded-lg border border-border px-3 py-2 bg-default-100/40"
            >
      ...
      {showAddForm ? (
        <div className="space-y-2 rounded-lg border border-border p-3 bg-default-100/30">
          <Input
            label="Asset ID"
            type="number"
            size="sm"
