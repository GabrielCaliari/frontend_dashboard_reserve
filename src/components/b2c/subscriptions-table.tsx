...
import { useListB2CSubscriptions, useCancelB2CSubscription } from'@/src/common/hooks/useB2CSubscriptions';
...
export function B2CSubscriptionsTable() {
 ...
 const colors: Record<UserSubscription['status'],'success' |'warning' |'danger' |'default'> = {
 active:'success',
 trialing:'warning',
 canceled:'default',
 incomplete:'warning',
 ...
 const statusKey =`status${status.charAt(0).toUpperCase() + status.slice(1).replace('_','')}` as any;
 return t(statusKey);
 };
 ...
 onPress={() =>
 window.open(`https://dashboard.stripe.com/subscriptions/${subscription.stripeSubscriptionId}`,'_blank'
 )
 }
 >
 {t('viewOnStripe')}
 ...
 message={t('confirmCancelMessage', { email: selectedSubscription?.studentEmail ||'' })}
