...
import { MoreVertical, Calendar, Clock, User, Package, CheckCircle, XCircle } from "lucide-react";
...
export function AppointmentsTable({ appointments, isLoading, onRefresh }: AppointmentsTableProps) {
  ...
  const renderCell = (appointment: Appointment, key: string) => {
    switch (key) {
      case "lead":
        return (
          <div className="flex flex-col gap-1">
            ...
            <span className="text-xs text-muted-foreground">{appointment.Lead?.email || "—"}</span>
            {appointment.Lead?.phone_number && (
              <span className="text-xs text-muted-foreground">{appointment.Lead.phone_number}</span>
            )}
          </div>
        );
      case "date":
      ...
      case "status":
        return (
          <Chip color={STATUS_COLORS[appointment.status]} variant="flat" size="sm">
            {STATUS_LABELS[appointment.status]}
          </Chip>
        );
      case "created":
        return <span className="text-sm text-muted-foreground">{formatDate(appointment.createdAt)}</span>;
      case "actions":
      ...
      >
        {(appointment) => (
          <TableRow key={appointment.id}>
            {(col) => <TableCell>{renderCell(appointment, col as string)}</TableCell>}
          </TableRow>
        )}
      </TableBody>
    </Table>
