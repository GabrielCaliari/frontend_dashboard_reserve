...
import { Button, Input, Select, SelectItem, Checkbox, Card, CardBody, Divider } from"@heroui/react";
import { Save, Clock, Calendar } from"lucide-react";
import { useTranslations } from"next-intl";
import { toast } from"react-hot-toast";
import { useScheduleConfig, useSaveScheduleConfig } from"@/src/common/hooks/appointments/use-schedule-config";
...
export function ScheduleConfigForm() {
 ...
 toast.error(error.response?.data?.message || error.message || t("errorSaving"));
 }
 }
 };
 ...
 return (
 <form onSubmit={handleSubmit} className="space-y-6 p-6 bg-default-100 rounded-xl border border-divider">
 ...
 onChange={(e) => setConfig({ ...config, capacityPerSlot: Number(e.target.value) })}
 min={1}
 required
 />
 </div>
 ...
 return (
 <Card key={weekday.value} className={dayConfig.enabled ?"border-primary/20" :""}>
 ...
 onValueChange={(enabled) => updateDay(weekday.value, { enabled })}
 >
 ...
 onChange={(e) => updateDay(weekday.value, { workStartTime: e.target.value })}
 />
 <Input
 type="time"
 label={t("workEndTime")}
 ...
 onChange={(e) => updateDay(weekday.value, { workEndTime: e.target.value })}
 />
 <Input
 type="time"
 ...
 onChange={(e) => updateDay(weekday.value, { lunchStartTime: e.target.value })}
 />
 <Input
 type="time"
 ...
 onChange={(e) => updateDay(weekday.value, { lunchEndTime: e.target.value })}
 />
 </div>
 )}
 </div>
