...
export function AppointmentDetailModal({
  ...
  const hasLeadData = appointment.Lead && (
  ...
  const hasValidEmail = appointment.Lead?.email && 
  ...
  const hasValidPhone = appointment.Lead?.phone_number && 
      ...
      <ModalContent>
        <ModalHeader className="flex flex-col gap-1">
          <div className="flex items-center justify-between gap-4 pr-8">
            ...
            >
              {t(`status${appointment.status.charAt(0).toUpperCase() + appointment.status.slice(1)}` as any)}
            </Chip>
          </div>
        </ModalHeader>
          ...
          <div className="space-y-6">
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="font-medium text-sm break-all">{appointment.Lead?.email}</p>
                        </div>
                      </div>
                    </>
                  )}
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="font-medium">{appointment.Lead?.phone_number}</p>
                        </div>
                      </div>
                    </>
                  )}
              ...
              <div className="space-y-3">
                ...
                <div className="bg-warning/10 rounded-xl p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-warning">{t("leadDataUnavailable")}</p>
                      <p className="text-sm text-muted-foreground mt-1">
                        Lead ID: <span className="font-mono">{appointment.leadId}</span>
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-default-100 rounded-xl p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <p className="font-semibold">{appointment.selectedProduct}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="pt-2">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {t("createdAt")}: {new Date(appointment.createdAt).toLocaleString(locale === "pt" ? "pt-BR" : "en-US")}
              </p>
            </div>
          </div>
        </ModalBody>
          ...
          {appointment.status === "confirmed" && (
            <>
              <Button
                ...
                spinner={<div className="w-4 h-4 border-2 border-success border-t-transparent rounded-full animate-spin" />}
              >
                {!completeMutation.isPending && <CheckCircle className="w-4 h-4 mr-2" />}
                {t("actionComplete")}
              </Button>
              <Button
                color="danger"
                ...
                spinner={<div className="w-4 h-4 border-2 border-danger border-t-transparent rounded-full animate-spin" />}
              >
                {!cancelMutation.isPending && <XCircle className="w-4 h-4 mr-2" />}
                {t("actionCancel")}
              </Button>
            </>
          )}
