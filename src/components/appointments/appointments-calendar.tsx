...
export function AppointmentsCalendar({
  ...
  const monthLabel = viewDate.toLocaleDateString(dateLocale, {
    month: "long",
    year: "numeric",
  });
 ...
 ${todayDay && !selected ? "border-primary/40 font-bold" : ""}`}
            >
              <span
                className={`text-xs font-medium ${todayDay ? "text-primary" : "text-foreground"}`}
              >
                {day}
              </span>
      ...
      <div className="flex items-center gap-4 pt-3 border-t border-divider mt-3">
        ...
        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
          <span className="w-2 h-2 rounded-full bg-default-100" />{" "}
          {t("statusCancelled")}
        </div>
        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
