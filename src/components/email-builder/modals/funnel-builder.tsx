...
export function FunnelBuilder({
  ...
  const removeStep = (stepId: string) => {
    ...
    if (parentStep) {
      ...
      setSteps((prevSteps) =>
        prevSteps.map((step) =>
          step.id === parentStep.id
            ? { ...step, children: step.children.filter((id) => id !== stepId) }
            : step,
        ),
      );
    }
    ...
    const removeStepAndChildren = (id: string) => {
      ...
      const childrenToRemove = childrenIds.flatMap((childId) =>
        removeStepAndChildren(childId),
      );
    ...
    setSteps((prevSteps) =>
      prevSteps.filter((step) => !idsToRemove.includes(step.id)),
    );
 ...
 ${editingStepId === step.id ? "border-emerald-500" : "border-border"}
 ${step.type === "email" ? "bg-blue-50" : step.type === "condition" ? "bg-purple-50" : "bg-amber-50"}`}
        >
          ...
          <div className="flex-1">
            ...
            {step.type === "condition" && step.condition && (
              <div className="text-xs text-muted-foreground">
                <p>
                  <strong>{t("funnel.condition")}</strong>{" "}
                  {step.condition.type === "opened"
                    ? t("funnel.openedEmail")
                    : t("funnel.clickedLink")}
                </p>
                {step.condition.target && (
                  <p>
                    <strong>{t("funnel.targetEmail")}</strong>
                    {""}
                    {steps.find((s) => s.id === step.condition?.target)?.name ||
                      t("funnel.notDefined")}
                  </p>
                )}
              </div>
            ...
            {step.type === "delay" && step.delay && (
              <div className="text-xs text-muted-foreground">
                <p>
                  <strong>{t("funnel.delay")}</strong> {step.delay.days}{" "}
                  {t("funnel.daysAnd")} {step.delay.hours} {t("funnel.hours")}
                </p>
              </div>
            )}
          ...
          <div className="mt-2 pl-4 border-l-2 border-border">
            {step.children.map((childId) => (
              <div key={childId} className="relative">
                <div className="absolute -left-4 top-4 w-4 h-0.5 bg-default-100"></div>
  ...
  const renderEditPanel = () => {
    ...
    return (
      <div className="border-l border-border p-4 h-full overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          {step.type === "condition" && step.condition && (
            <>
              ...
              <div>
                <Label htmlFor="condition-target">
                  {t("funnel.targetEmailLabel")}
                </Label>
                <Select
                  value={step.condition.target || ""}
                  onValueChange={(value) =>
                    updateStep(step.id, {
                      condition: { ...step.condition, target: value },
                    })
                  }
                >
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="py-4 flex flex-col flex-grow overflow-hidden">
        ...
        <div className="flex flex-grow overflow-hidden border rounded-md">
          <div
            className={`${editingStepId ? "w-2/3" : "w-full"} overflow-y-auto p-4`}
          >
            {renderStep(rootStepId)}
          </div>
          {editingStepId && (
            <div className="w-1/3 overflow-y-auto">{renderEditPanel()}</div>
          )}
        </div>
      </div>
