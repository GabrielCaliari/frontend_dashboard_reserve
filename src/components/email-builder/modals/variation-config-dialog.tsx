...
export function VariationConfigDialog({
  isOpen,
  onClose,
  campaignId,
  copyVariationType,
}: VariationConfigDialogProps) {
  const t = useTranslations();
  const router = useRouter();
  const [variationType, setVariationType] = useState<ECopyVariationType | null>(
    copyVariationType,
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await updateEmailCampaignCopyVariant(
        campaignId,
        variationType,
      );
          ...
          {!success ? (
            <div className="space-y-4">
              ...
              <RadioGroup
                ...
                onValueChange={(value) =>
                  setVariationType(value as unknown as ECopyVariationType)
                }
                className="space-y-3"
              >
                <div className="flex items-start space-x-2 rounded-md border p-3 hover:bg-default-100 transition-colors">
                  <RadioGroupItem
                    value={String(ECopyVariationType.ai)}
                    id="ai"
                    className="mt-1"
                  />
                ...
                <div className="flex items-start space-x-2 rounded-md border p-3 hover:bg-default-100 transition-colors">
                  <RadioGroupItem
                    value={String(ECopyVariationType.manual)}
                    id="manual"
                    className="mt-1"
                  />
                  <div className="flex-1">
          ...
          {!success ? (
            <>
              ...
              <Button
                ...
                disabled={!variationType || isSubmitting}
              >
