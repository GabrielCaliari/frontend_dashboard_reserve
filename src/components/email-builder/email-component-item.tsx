...
export default function EmailComponentItem({
  ...
  const [{ handlerId }, drop] = useDrop({
    ...
    hover(item: any, monitor) {
      ...
      // Handle multi-component drag
        ...
        if (
          (hoverIndex < minDragIndex && isAboveMiddle) ||
          (hoverIndex > maxDragIndex && !isAboveMiddle)
        ) {
  ...
  return (
    ...
    >
      <Card
        className={`border-2 ${isSelected ? "border-blue-500" : "hover:border-blue-300"}`}
      >
        <CardContent className="p-4">
          <div className="absolute top-2 left-2 z-10">
            <Checkbox
              checked={isSelected}
              onCheckedChange={() => onToggleSelection(component.id)}
              className="h-4 w-4"
            />
          </div>
