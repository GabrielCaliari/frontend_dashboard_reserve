 ...
 */
export function AssetUpload({ collection, onUploadComplete }: AssetUploadProps) {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop,
    multiple: true,
    accept: collection.allowed_mime_types.reduce((acc, type) => {
      acc[type] = [];
      return acc;
  ...
  const uploadFile = useCallback(async (item: UploadQueueItem) => {
    ...
    try {
      ...
      setUploadQueue(prev =>
        prev.map(i =>
          i.id === item.id
            ? { ...i, status: "error", error: error.message || t("uploadError") }
            : i
        )
      );
            ...
            {uploadQueue.map(item => (
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  {!isImageFile(item.file.type) && (
                    <div className="flex-shrink-0 w-16 h-16 bg-gray-800 rounded flex items-center justify-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {isImageFile(item.file.type) && item.status === "pending" && (
                      <Input
                        size="sm"
