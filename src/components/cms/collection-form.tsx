...
interface CollectionFormProps {
  ...
  onSubmit: (
    data: Omit<CollectionFormData, "max_file_size_unit"> & {
...
export function CollectionForm({
  ...
  const {
    ...
    defaultValues: {
      ...
      allowed_mime_types:
        collection?.allowed_mime_types || MIME_TYPE_GROUPS.image,
  ...
  const handleFormSubmit = (data: CollectionFormData) => {
    const unit = FILE_SIZE_UNITS.find(
      (u) => u.value === data.max_file_size_unit,
    );
    const maxFileSizeBytes = Math.round(
      data.max_file_size * (unit?.multiplier || 1),
    );
              ...
              <div className="flex flex-wrap gap-2">
                {items.map((item) => (
                  <Chip key={item.key} size="sm">
                    {COMMON_MIME_TYPES.find((m) => m.value === item.key)
                      ?.label || item.key}
                  </Chip>
                ))}
              </div>
