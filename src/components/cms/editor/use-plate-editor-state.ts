...
export function usePlateEditorState({
  ...
  const editor = usePlateEditor({
    ...
    value:
      typeof parsedInitial === "string" && parsedInitial
  ...
  const analyzeTimerRef = React.useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
      ...
      try {
        ...
        const message =
          err instanceof Error
            ? err.message
            : "Invalid markdown in initial content.";
    ...
    analyzeTimerRef.current = setTimeout(() => {
      try {
        ...
        try {
          ...
          console.warn(
            "[PlateEditor] Serialize failed during analysis, keeping last markdown:",
            serErr,
          );
        }
        setMarkdownContent(md);
      ...
      setTimeout(() => {
        try {
          ...
          if (newMode === "formatted") {
            ...
            announce(
              "Markdown contains errors. Fix them before leaving source view.",
            );
          }
    ...
    (md: string) => {
      ...
      analyzeTimerRef.current = setTimeout(() => {
        ...
        try {
          ...
          const message =
            err instanceof Error ? err.message : "Invalid markdown syntax.";
