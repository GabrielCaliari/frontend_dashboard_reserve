...
import { EditorToolbar } from"@/src/components/cms/editor/editor-toolbar";
import { usePlateEditorState } from"@/src/components/cms/editor/use-plate-editor-state";
import type { PlateEditorProps } from"@/src/components/cms/editor/editor-types";
...
export function PlateEditor({
 ...
 className={cn("w-full flex items-center gap-2 px-3 py-2 rounded-md text-left transition-colors","hover:bg-accent/50",
 ...
 {viewMode ==="formatted" && (
 <div
 className={cn("flex-1 min-h-0 flex flex-col transition-opacity duration-200",
 isTransitioning ?"opacity-0" :"opacity-100",
 )}
 >
 <div className="flex-1 min-h-0 overflow-y-auto bg-content1">
 <div
 className={cn("max-w-3xl mx-auto px-6 sm:px-8 py-8 bg-content1",
 ...
 if (leaf.italic) node = <em className="italic">{node}</em>;
 if (leaf.underline) node = <u className="underline">{node}</u>;
 if (leaf.strikethrough) node = <s className="line-through">{node}</s>;
 if (leaf.code) {
 node = (
