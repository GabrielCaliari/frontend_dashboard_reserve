...
import type { CollectionType, MediaAsset } from "@/src/common/@types/@cms-media";
...
export function BlogImageInsertDialog({
  ...
  const blogs = React.useMemo(() => (Array.isArray(blogsData) ? blogsData : []), [blogsData]);
  ...
  } = useCollection(mediaCollectionId ?? "");
  ...
  const assets = assetsData?.data?.filter((asset) => asset.mime_type.startsWith("image/")) ?? [];
  ...
  const handleInsert = React.useCallback(() => {
    if (selectedAsset) {
      ...
      onInsert?.(selectedAsset.url, altText.trim() || selectedAsset.alt_text || "");
      onOpenChange(false);
      return;
    }
  ...
  }, [allowExternalUrl, altText, imageUrl, onInsert, onOpenChange, onSelectAsset, selectedAsset]);
  ...
  const typeBadge = collection ? TYPE_BADGES[collection.type] ?? { color: "primary" as const } : null;
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 sm:px-6">
          ...
          {hasLinkedCollection ? (
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px] xl:items-start">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {collection && typeBadge && (
                      <Chip size="sm" color={typeBadge.color} variant="flat" className="capitalize">
                        {collection.type}
                      </Chip>
                    )}
                  </div>
              ...
              {collection && (
                <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs text-muted-foreground">
                  ...
                  {collection.max_items && (
                    <span>
                      Max items:{" "}
                      <span className="font-medium text-foreground/80">{collection.max_items}</span>
                    </span>
                  )}
                  <span>
                    Allowed types:{" "}
              ...
              <div className="space-y-4 rounded-md border border-border bg-muted/30 p-4 xl:sticky xl:top-0">
                <div>
                  <div className="text-sm font-medium text-foreground">Selected image</div>
                  <div className="text-xs text-muted-foreground">
                    Images are loaded from the media collection configured on the blog. You can also paste an external URL below if needed.
                  </div>
                </div>
                ...
                {selectedAsset ? (
                  <>
                    ...
                    <div className="space-y-1 text-xs text-muted-foreground">
                      <div className="truncate text-sm font-medium text-foreground">{selectedAsset.filename}</div>
            ...
            <div className="rounded-md border border-border bg-muted/20 p-4 text-sm text-muted-foreground">
              This article's blog does not have a media collection configured. Use an external URL below, or configure a collection on the blog first.
            </div>
          )}
            ...
            <div className="rounded-md border border-border bg-muted/20 p-4">
              ...
              <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
                ...
                <div className="rounded-md border border-border bg-background p-4 text-sm text-muted-foreground">
                  Keep this as a fallback only. The primary authoring flow is the blog collection above, because it preserves CMS validation and media governance.
                </div>
              </div>
            </div>
          )}
