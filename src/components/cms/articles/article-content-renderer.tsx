        ...
        components={{
          ...
          img: ({ node: _node, className: imageClassName, alt, ...props }) => (
            <img
              ...
              loading="lazy"
              className={cn(
                "my-6 w-full rounded-lg border border-border bg-default-100 object-cover",
                imageClassName,
              )}
            />
          ),
          ...
          td: ({ node: _node, className: cellClassName, ...props }) => (
            <td
              {...props}
              className={cn(
                "border border-border px-4 py-2 align-top",
                cellClassName,
              )}
            />
          ),
          pre: ({ node: _node, className: preClassName, ...props }) => (
            <pre
              {...props}
              className={cn(
                "overflow-x-auto rounded-lg border border-border bg-default-100 p-4",
                preClassName,
              )}
            />
          ),
          code: ({ node: _node, className: codeClassName, ...props }) => (
            <code
              {...props}
              className={cn(
                "rounded bg-default-100 px-1.5 py-0.5",
                codeClassName,
              )}
            />
          ),
