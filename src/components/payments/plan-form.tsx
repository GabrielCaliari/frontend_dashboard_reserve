...
import { z } from"zod";
import { Button, Input, Select, SelectItem, Textarea, Switch } from"@heroui/react";
...
import type { StripePlan, PlanBillingInterval, PlanCurrency } from"@/src/common/@types/@payments";
...
export function PlanForm({ existing, onSuccess, onCancel }: PlanFormProps) {
 ...
 credits_released_trial_period: existing?.credits_released_trial_period ?? 0,
 },
 });
 ...
 value={field.value}
 onValueChange={(value) => {
 const formatted = formatCurrencyInput(value, watch("currency"));
