...
export function B2BProductsTab({ refreshKey: _refreshKey }: B2BProductsTabProps) {
 ...
 <p className="text-xs text-muted-foreground">{t('noB2BProductsDesc')}</p>
 </div>
 );
 }
 ...
 className="w-48"
 classNames={{ trigger:'border-border bg-default-100/50 h-8 min-h-8' }}
 >
 ...
 <code className="text-xs text-muted-foreground font-mono">{product.slug}</code>
 {product.description && (
 ...
 <CopyButton value={product.stripePriceId} label={t('copyPriceId')} />
 ) : (
 <span className="text-xs text-muted-foreground">—</span>
 )}
 </div>
 ...
 <Button size="sm" variant="light" isIconOnly onPress={() => setFeesProduct(product)} aria-label={t('feesConfigureBtn')}>
 ...
 <B2BGlobalFeesModal isOpen={isGlobalFeesOpen} onClose={() => setIsGlobalFeesOpen(false)} />
