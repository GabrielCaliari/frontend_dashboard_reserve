...
function CategoryRow({ category, onUpdate, onDelete }: {
 ...
 <Input size="sm" label={t('feesGlobalChipCostLabel')} placeholder={t('feesCategoriesInheritGlobal')} value={chipCost} onValueChange={setChipCost} type="number" min="0" step="0.1" startContent={<Percent className="w-3 h-3 text-muted-foreground" />} classNames={{ inputWrapper:'bg-white/[0.03] border-white/[0.07]' }} />
 <Input size="sm" label={t('feesGlobalShippingLabel')} placeholder={t('feesCategoriesInheritGlobal')} value={shipping} onValueChange={setShipping} type="number" min="0" step="0.01" startContent={<Truck className="w-3 h-3 text-muted-foreground" />} classNames={{ inputWrapper:'bg-white/[0.03] border-white/[0.07]' }} />
 </div>
...
export function CategoriesModal({ isOpen, onClose }: CategoriesModalProps) {
 ...
 classNames={{ base:'bg-background border border-white/10', header:'border-b border-white/[0.07]', footer:'border-t border-white/[0.07]', body:'max-h-[60vh]' }}>
 <ModalContent>
 <ModalHeader className="flex items-center gap-2.5">
 <div className="w-7 h-7 rounded-lg bg-primary/15 flex items-center justify-center">
 <Tags className="w-3.5 h-3.5 text-primary" />
 </div>
 <span className="text-sm font-semibold text-foreground">{t('feesCategoriesModalTitle')}</span>
 {categories.length > 0 && (
 ...
 <div className="grid grid-cols-2 gap-2">
 <Input size="sm" label={t('feesGlobalChipCostLabel')} placeholder={t('feesCategoriesInheritGlobal')} value={newChipCost} onValueChange={setNewChipCost} type="number" min="0" step="0.1" startContent={<Percent className="w-3 h-3 text-muted-foreground" />} classNames={{ inputWrapper:'bg-white/[0.03] border-white/[0.07]' }} />
 <Input size="sm" label={t('feesGlobalShippingLabel')} placeholder={t('feesCategoriesInheritGlobal')} value={newShipping} onValueChange={setNewShipping} type="number" min="0" step="0.01" startContent={<Truck className="w-3 h-3 text-muted-foreground" />} classNames={{ inputWrapper:'bg-white/[0.03] border-white/[0.07]' }} />
 </div>
 ...
 <p className="text-center text-sm text-muted-foreground py-6">{t('feesCategoriesEmpty')}</p>
 ) : (
