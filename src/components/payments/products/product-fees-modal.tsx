...
export function ProductFeesModal({
  ...
  useEffect(() => {
    if (isOpen && product) {
      ...
      setChipOverride(
        product.chipCostOverride != null
          ? String(product.chipCostOverride)
          : "",
      );
      setShippingOverride(
        product.shippingFeeOverride != null
          ? String((product.shippingFeeOverride / 100).toFixed(2))
          : "",
      );
  ...
  const handleSave = async () => {
    ...
    await updateFees.mutateAsync({
      ...
      data: {
        ...
        shippingFeeOverride:
          shippingOverride !== ""
            ? Math.round(parseFloat(shippingOverride) * 100)
            : null,
        requiresShipping,
      },
    });
      ...
      <ModalContent>
        ...
        <ModalBody className="py-5 space-y-4">
          ...
          <div className="space-y-1.5">
            <Select
              label={t("feesProductCategoryLabel")}
              placeholder={t("feesProductCategoryPlaceholder")}
              ...
              <div className="flex gap-2 px-1">
                {selectedCategory.chipCostPercent != null ? (
                  <span className="text-[11px] text-primary/70">
                    {t("feesChipCost")}: {selectedCategory.chipCostPercent}%
                  </span>
                ) : (
                  <span className="text-[11px] text-muted-foreground italic">
                    {t("feesChipCost")}: {t("feesCategoriesInheritGlobal")}
                  </span>
                )}
                {selectedCategory.shippingFee != null ? (
                  <span className="text-[11px] text-blue-400/70">
                    {t("feesShipping")}: R$
                    {(selectedCategory.shippingFee / 100).toFixed(2)}
                  </span>
                ) : (
                  <span className="text-[11px] text-muted-foreground italic">
                    {t("feesShipping")}: {t("feesCategoriesInheritGlobal")}
                  </span>
                )}
              </div>
            )}
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm text-foreground">
                  {t("feesProductPhysicalLabel")}
                </p>
                <p className="text-xs text-muted-foreground">
                  {t("feesProductPhysicalDesc")}
                </p>
              </div>
            </div>
            <Switch
              isSelected={requiresShipping}
              onValueChange={setRequiresShipping}
              size="sm"
            />
          </div>
          ...
          <div className="space-y-1">
            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground px-1">
              {t("feesProductOverridesTitle")}
            </p>
            <div className="space-y-3">
              <Input
                label={t("feesProductChipOverrideLabel")}
                placeholder={t("feesProductChipOverridePlaceholder")}
                value={chipOverride}
                onValueChange={setChipOverride}
                type="number"
                min="0"
                step="0.1"
                startContent={
              ...
              />
              <Input
                label={t("feesProductShippingOverrideLabel")}
                placeholder={t("feesProductShippingOverridePlaceholder")}
        ...
        <ModalFooter className="gap-2">
          <Button
            ...
            variant="flat"
            onPress={onClose}
            isDisabled={updateFees.isPending}
          >
            {tCommon("cancel")}
          </Button>
          <Button
            ...
            color="primary"
            onPress={handleSave}
            isLoading={updateFees.isPending}
          >
            {tCommon("save")}
          </Button>
        </ModalFooter>
