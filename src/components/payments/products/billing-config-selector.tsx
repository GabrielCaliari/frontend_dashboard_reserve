...
const PRESET_MAP: Record<
  Exclude<BillingPreset, "custom">,
  { interval: BillingInterval; intervalCount: number }
> = {
...
export function BillingConfigSelector({
  ...
  const isRecurring =
    value.mode === "recurring_infinite" || value.mode === "recurring_limited";
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {billingModes.map((mode) => {
            ...
            return (
              ...
              >
                <span
                  className={isActive ? mode.color : "text-muted-foreground"}
                >
                  {mode.icon}
                </span>
                <span
        ...
        <div
          className={`mt-2 p-2.5 rounded-lg border ${selectedMode.activeBg} ${selectedMode.activeBorder}`}
        >
          <p className={`text-xs ${selectedMode.color}`}>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all border ${
                    isActive
                      ...
                      : "bg-default-100 border-[#2a2a3e] text-muted-foreground hover:border-[#3a3a4e] hover:text-foreground"
                  } ${isDisabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"} ${
          ...
          {preset === "custom" && (
            <div className="grid grid-cols-2 gap-2 mt-2">
              ...
              <Input
                ...
                onValueChange={(v) =>
                  onChange({ ...value, intervalCount: parseInt(v) || 1 })
                }
                isDisabled={isDisabled}
                size="sm"
      ...
      {value.mode === "recurring_limited" && (
        <div className="space-y-2">
          <Input
            ...
            onValueChange={(v) =>
              onChange({ ...value, maxCharges: parseInt(v) || 1 })
            }
            isDisabled={isDisabled}
            size="sm"
            classNames={{ inputWrapper: "bg-default-100 border-[#2a2a3e]" }}
              ...
              <div className="flex items-start gap-2">
                ...
                <p className="text-xs text-amber-300/80">
                  {t("limitedInfoDesc", {
                    ...
                    interval: t(
                      `interval${value.interval.charAt(0).toUpperCase() + value.interval.slice(1)}`,
                    ),
                  })}
                </p>
              </div>
        ...
        <div className="space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <Input
              ...
              onValueChange={(v) =>
                onChange({ ...value, accessDuration: parseInt(v) || 1 })
              }
              isDisabled={isDisabled}
              size="sm"
              classNames={{ inputWrapper: "bg-default-100 border-[#2a2a3e]" }}
            ...
            <Select
              ...
              onChange={(e) =>
                onChange({
                  ...value,
                  accessDurationUnit: e.target.value as
                    | "day"
                    | "month"
                    | "year",
                })
              ...
              <div className="flex items-start gap-2">
                ...
                <p className="text-xs text-purple-300/80">
                  {t("oneTimeInfoDesc", {
                    ...
                    unit: t(
                      `interval${(value.accessDurationUnit || "month").charAt(0).toUpperCase() + (value.accessDurationUnit || "month").slice(1)}`,
                    ),
                  })}
                </p>
              </div>
