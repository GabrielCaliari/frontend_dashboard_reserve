...
export function LanguageSwitcher() {
 ...
 <span className="font-bold text-xs uppercase tracking-widest text-foreground">{current.code}</span>
 ...
 <DropdownTrigger>
 <Button variant="ghost" size="sm" className="flex items-center gap-2 px-3 py-2 hover:bg-foreground/10 transition-colors rounded-full border border-border">
 <ReactCountryFlag countryCode={current.country} svg className="rounded-full" style={{ width: 16, height: 16, objectFit:'cover' }} />
 <span className="font-bold text-xs uppercase tracking-widest text-foreground">{current.code}</span>
 <ChevronDown className="h-4 w-4 text-foreground" />
 </Button>
 ...
 locale === lang.code ?'bg-foreground/10' :'hover:bg-foreground/10 focus:bg-foreground/10'
 }`}
 >
